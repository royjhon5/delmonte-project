...
import { OPEN_ARCHIVEBYDEPT } from '../../store/actions';
...
const SummaryByDeptartmentAndDate = ({ maxWidth, onClick }) => {
    const theme = useTheme();
    const open = useSelector((state) => state.customization.openArchiveByDept);
    const dispatch = useDispatch();
    const CloseDialog = () => { dispatch({ type: OPEN_ARCHIVEBYDEPT, openArchiveByDept: false }) }
    const [loadSaving, setLoadSaving] = useState(false);
    const { data: deptList } = hookContainer('/get-department');
    ...
    const generateReport = async (event) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                window.open(linkToBackend + "/print-summary-by-date-range?dataVariable[Department]=" + dataVariable.Department +
    ...
    return (
        <>
            <Dialog
                ...
                <DialogTitle>
                    <Typography fontSize={20}>Summary of Archived By Department and Date Range</Typography>
                </DialogTitle>
                    ...
                    >
                        ...
                        <Grid container spacing={2} sx={{ mt: .5 }}>
                            <Grid item xs={12} md={12}>
                                <TextField variant="outlined" size="medium" label="Department" select value={dataVariable.Department} onChange={updateDataVariable} name="Department" SelectProps={{ native: true, }} fullWidth required>
                                    <option value=""></option>
                                    {deptList?.map((option) => (
...
SummaryByDeptartmentAndDate.propTypes = {
    maxWidth: PropTypes.oneOf(['xs', 'sm', 'md', 'lg', 'xl']),
    onClick: PropTypes.func,
}

export default SummaryByDeptartmentAndDate