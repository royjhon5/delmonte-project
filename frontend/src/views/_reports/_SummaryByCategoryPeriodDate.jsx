...
import { OPEN_ARCHIVEBYCAT } from '../../store/actions';
...
const SummaryByCategoryPeriodDate = ({ maxWidth, onClick }) => {
    const theme = useTheme();
    const open = useSelector((state) => state.customization.openArchiveByCat);
    const dispatch = useDispatch();
    const CloseDialog = () => { dispatch({ type: OPEN_ARCHIVEBYCAT, openArchiveByCat: false }) }
                    ...
                    >
                        ...
                        <Grid container spacing={2} sx={{ mt: .5 }}>
                            <Grid item xs={12} md={12}>
                                <TextField variant="outlined" size="medium" label="Document Type" select value={dataVariable.DocTypeLinkID} onChange={updateDataVariable} name="DocTypeLinkID" SelectProps={{ native: true, }} fullWidth required>
                                    <option value=""></option>
                                    {docList?.map((option) => (
...
SummaryByCategoryPeriodDate.propTypes = {
    maxWidth: PropTypes.oneOf(['xs', 'sm', 'md', 'lg', 'xl']),
    onClick: PropTypes.func,
}

export default SummaryByCategoryPeriodDate