...
import AddActivityListModal from "./AddActivityListModal";
...
const ActivityListData = () => {
    ...
    const ColumnHeader = [
        {
            field: 'activityname', headerName: 'Activity Name', width: 900,
            renderCell: (data) => (
    ...
    const openAddActivityListModal = () => {
        dispatch({ type: OPEN_CUSTOM_MODAL, openCustomModal: true });
    }

    const refreshData = () => queryClient.invalidateQueries(['/get-activity']);
    ...
    const deleteData = useMutation({
        mutationFn: () => http.delete(`/remove-activity?id=${selectedID}`),
        onSuccess: () => {
    ...
    return (
        <>
            <AddActivityListModal RefreshData={refreshData} />
            ...
            <Paper>
                <Stack sx={{ display: 'flex', padding: '20px', flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <Button variant="contained" onClick={openAddActivityListModal}>Add Activity</Button>
                </Stack>
                <CustomDataGrid
