...
const AccountMasterData = () => {
    ...
    const { data: mainData } = hookContainer('/get-accounttocharge');
        ...
        },
        {
            ...
            renderCell: (data) => {
                const SelectData = () => {
                    const obj = {
                        id: data.row.id,
                        activity_id_link: data.row.activity_id_link,
                        glcode_id_link: data.row.glcode_id_link,
                        costcenter_id_link: data.row.costcenter_id_link,
                    };
    ...
    const openAddAccountMasterModal = () => {
        dispatch({ type: OPEN_CUSTOM_MODAL, openCustomModal: true });
    }

    const refreshData = () => queryClient.invalidateQueries(['/get-accounttocharge']);
    ...
    const deleteData = useMutation({
        mutationFn: () => http.delete(`/remove-accounttocharge?id=${selectedID}`),
        onSuccess: () => {
    ...
    return (
        <>
            <AddAccountMasterModal RefreshData={refreshData} />
