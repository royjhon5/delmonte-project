...
const CostCenterData = () => {
    ...
    const { data: mainData } = hookContainer('/get-costcenter');
    ...
    const ColumnHeader = [
        {
            field: 'costcenter', headerName: 'Cost Center Name', flex: 1,
            renderCell: (data) => (
        ...
        },
        {
            ...
            renderCell: (data) => {
                const SelectData = () => {
                    const obj = {
                        id: data.row.id,
                        costcenter: data.row.costcenter,
                    };
    ...
    const openAddCostCenterModal = () => {
        dispatch({ type: OPEN_CUSTOM_MODAL, openCustomModal: true });
    }

    const refreshData = () => queryClient.invalidateQueries(['/get-costcenter']);
    ...
    const deleteData = useMutation({
        mutationFn: () => http.delete(`/remove-costcenter?id=${selectedID}`),
        onSuccess: () => {
    ...
    return (
        <>
            <AddCostCenterModal RefreshData={refreshData} />
            ...
            <Paper>
                <Stack sx={{ display: 'flex', padding: '20px', flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <Button startIcon={<AddIcon />} variant="contained" onClick={openAddCostCenterModal}>ADD NEW</Button>
                </Stack>
                <CustomDataGrid
