...
import SearchPersonnelModal from "../_upload_financial_documents/SearchPersonnelModal.jsx";
import { useQueryClient } from "@tanstack/react-query";
import { useAuth } from '../../../modules/context/AuthContext';
import SearchIcon from '@mui/icons-material/Search';

const OfficeDocumentsData = () => {
    const documentType = "Accounting Office Documents";
    ...
    const { data: subDocList } = hookContainer('get-sub-document-type', { DocType_IDLink: 2 });
    ...
    const initialDataVariable = {
        id: "",
        DocTypeLinkID: 2, // office Documents 
        DocReferenceID: "AOD" + getDateNow('year') + getDateNow('month'),
        DVNo: "",
        DVDate: "",
        JEVNo: "",
        ORSNo: "",
        ...
        try {
            ...
            if (saveDataResponse.data.success) {
                await http.post('/add-audit-log', {
                    ...
                    interface: "Upload Office Documents"
                }); // save audit log
    ...
    const [officeType, setOfficeType] = useState(0);
    async function modalCloseOffice(params) {
        ...
        if (officeType == 2) {
            setDataVariable(prevState => ({
                ...prevState,
                OfficeTo: params.description,
            }));
        }
    }

    const [openModalPersonnel, setOpenModalPersonnel] = useState(false);
    async function modalClosePersonnel(params) {
        setDataVariable(prevState => ({
            ...prevState,
            personnelName: params.fullname,
        }));
    }
    ...
    return (
        <>
            ...
            />
            <SearchPersonnelModal
                openModal={openModalPersonnel}
                setOpen={() => { setOpenModalPersonnel(false) }}
                onCloseModal={modalClosePersonnel}
            />
            <Grid container spacing={1}>
                <Grid item xs={12} md={6}>
                    <Paper sx={{ padding: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                        ...
                        <Grid container spacing={1}>
                            <Grid item xs={12} md={6}>
                                <TextField variant="filled" type="date" fullWidth size="small" label="Document Date" value={dataVariable.documentDate} onChange={updateDataVariable} name="documentDate" InputLabelProps={{ shrink: true }} />
                            </Grid>
                            <Grid item xs={12} md={6}>
                                <TextField variant="filled" fullWidth size="small" label="Document Nature" value={dataVariable.DocNature} onChange={updateDataVariable} name="DocNature" />
                            </Grid>
                        </Grid>
                        <Grid container spacing={1}>
                            <Grid item xs={12} md={6}>
                                <TextField variant="filled" fullWidth size="small" label="Office From" value={dataVariable.Office} onChange={updateDataVariable} name="Office" InputProps={{
                                    endAdornment: (
                                        <Button variant="contained" onClick={() => { setOpenModalOffice(true); setOfficeType(1); }}><SearchIcon /></Button>
                                    )
                                }} />
                            </Grid>
                            <Grid item xs={12} md={6}>
                                <TextField variant="filled" fullWidth size="small" label="Office To" value={dataVariable.OfficeTo} onChange={updateDataVariable} name="OfficeTo" InputProps={{
                                    endAdornment: (
                                        <Button variant="contained" onClick={() => { setOpenModalOffice(true); setOfficeType(2); }}><SearchIcon /></Button>
                                    )
                                }} />
                            </Grid>
                        </Grid>
                        ...
                        <Grid container spacing={1}>
                            ...
                            <Grid item xs={12} md={6}>
                                <TextField variant="filled" fullWidth size="small" label="Personnel Name" value={dataVariable.personnelName} onChange={updateDataVariable} name="personnelName" InputProps={{
                                    endAdornment: (
                                        <Button variant="contained" onClick={() => { setOpenModalPersonnel(true); setOfficeType(2); }}><SearchIcon /></Button>
                                    )
                                }} />
                            </Grid>
                            <Grid item xs={12} md={6}>
                                <TextField variant="filled" fullWidth size="small" label="Subject" />
                            </Grid>
                        </Grid>
                        <TextField variant="filled" fullWidth size="small" label="Keywords" multiline rows={4} value={dataVariable.KeyWord} onChange={updateDataVariable} name="KeyWord" />
                        ...
                        <TextField variant="filled" fullWidth size="small" label="File Size" value={dataVariable.fileSize} onChange={updateDataVariable} name="fileSize" inputProps={{ readOnly: true }} />
