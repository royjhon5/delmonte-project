import { Box, Button, Grid, Paper, TextField, Typography, Grow } from "@mui/material";
...
import SearchFromEcardingModal from "./SearchFromEcardingModal.jsx";
import SearchOfficeModal from "./SearchOfficeModal.jsx";
import SearchPayeeModal from "./SearchPayeeModal.jsx";
...
const FinancialDocumentsData = () => {
    ...
    const documentType = "Financial Documents";
    const [iframePath, setIframePath] = useState("");
    const [scannerPath, setScannerPath] = useState("");
    const [filePath, setFilePath] = useState("");
    const { data: scannerList } = hookContainer('get-scanner-list');
    const { data: subDocList } = hookContainer('get-sub-document-type', { DocType_IDLink: 1 });
    ...
    const initialDataVariable = {
        id: "",
        DocTypeLinkID: 1, // financial Documents 
        DocReferenceID: "FD" + getDateNow('year') + getDateNow('month'),
        DVNo: "",
        DVDate: dayjs().format('YYYY-MM-DD'),
        JEVNo: "",
        ORSNo: "",
        ORSDate: "",
        Payee: "",
        ...
        try {
            ...
            if (saveDataResponse.data.success) {
                await http.post('/add-audit-log', {
                    ...
                    interface: "Upload Financial Documents"
                }); // save audit log
                ...
                AppSocket.emit('triggerNewNotification');
                clearData();
    ...
    return (
        <>
            {loadSaving ? <Grow in={true}><div className="wrapper-bg"><LoadSaving title={loadSaving} /></div></Grow> : ''}
...
export default FinancialDocumentsData