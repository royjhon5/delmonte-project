import { Box, Button, Dialog, DialogContent, Grid, IconButton, ListItemText, TextField, Typography, useTheme } from "@mui/material"
import NoData from "../../../components/CustomDataTable/NoData";
import { useDispatch, useSelector } from "react-redux";
import { OPEN_COMPANYBRANCHSELECTION } from "../../../store/actions";
...
const CompanyBranchSelection = ({Dataobj}) => {
  const theme = useTheme();
  const open = useSelector((state) => state.customization.openCompanyBranchSelection);
  const dispatch = useDispatch();
  const CloseDialog = () => {dispatch({ type: OPEN_COMPANYBRANCHSELECTION, openCompanyBranchSelection: false })}
  const [search, setSearch] = useState('');
  const { data, loading } = useGetCompanyBranch();
  const constMappedData = Array.isArray(data) ? data.map((row) => {
    return { ...row, id: row.BrID };
  }) : [];

  const SearchFilter = (rows) => {
    return rows.filter(row =>
        row.CompName.toLowerCase().includes(search.toLowerCase()) || 
        row.BrName.toLowerCase().includes(search.toLowerCase())
    );
  };
  
  const ColumnHeader = [
    { field: 'CompName', headerName: 'Company Name', flex: 1,
      renderCell: (params) => (
        <ListItemText>
            {params.row.CompName}
    ...
    { field: "actions", headerAlign: 'right',
      ...
      renderCell: (params) => {
          const SelectData = () => {
            ...
            Dataobj(entireData);
            dispatch({ type: OPEN_COMPANYBRANCHSELECTION, openCompanyBranchSelection: false }); 
          };
      return (
        <Box>
...
CompanyBranchSelection.propTypes = {
