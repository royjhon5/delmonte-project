...
const NewCheckVoucher = ({onClose, open, onSaveData  }) => {
  ...
  const [preparedBy, setPreparedBy] = useState(accessToken.Fname || '');
  const [approvedBy, setApprovedBy] = useState(null);
  const [approvedByPos, setApprovedByPos] = useState(null);
  ...
  const getCode = useCallback(async () => {
    try {
      const response = await http.get('/series-code');
      const initialCode = response.data.latestId ? `${year}${month}-${response.data.latestId}` : `${year}${month}-1`;
      setSeriesCode(initialCode);
    } catch (error) {
      console.error(error);
    }
  }, [month, year]);

  const getCvSignatures = useCallback(async () => {
    try {
      const response = await http.get('/cvsignatures-details');
      setApprovedBy(response.data[0].ApprovedBy)
      setApprovedByPos(response.data[0].ApprovedByPos)
    } catch (error) {
      console.error(error);
    }
  }, []);
  ...
  const getDefaultBranch = useCallback(async () => {
    try {
      ...
      setBRanchCode(response.data[0].BrCode)
  ...
  useEffect(() => {
      ...
      getCvSignatures();
      getDefaultBranch();
  }, [getCode, month, year, getCvSignatures, getDefaultBranch])


  const saveNewCheckVoucher = () => {
    ...
    const voucherData = {
      SeriesNo: isSeriesNumberEditable ? customSeriesCode : seriesCode,
      Date: formattedDate,
      ...
      ApprovedByPos: approvedByPos,
      CheckPayee: chequePayee,
      CMonth: formattedMonth,
      CYear: formattedYear,
      RTIN: voucherTIN
    };
    addCheckVoucher.mutate(voucherData);
  };

  const addCheckVoucher = useMutation({
    mutationFn: (voucherData) => http.post('/new-checkvoucher', voucherData),
    onSuccess: (response, voucherData) => {
      toast.success(response.data.message);
      onSaveData(voucherData);
      getCode();
      closeModal();
    },
    onError: (error) => {
        ...
        <DialogContent>
          <Grid container spacing={1.5}>
            <Grid item xs={12} md={12}>
              <Typography variant='h4'>New Check Voucher</Typography>
            </Grid>
            ...
            <Grid item xs={12} md={12} sx={{ display: 'flex' , justifyContent:'flex-end', flexDirection: 'row', gap:1}}>
              <Button size='large' variant='contained' onClick={saveNewCheckVoucher}>Set Check Voucher</Button>
...
NewCheckVoucher.propTypes = {
  onSaveData: PropTypes.func.isRequired,
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
}

export default NewCheckVoucher
