...
import AddScanner from "./_AddScanner";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import DeleteSwal from "../../../components/Swal/DeleteSwal";
import http from "../../../api/http";
import { toast } from "sonner";

const ScannerListData = () => {
    const dispatch = useDispatch();
    const [search, setSearch] = useState('');
    const { data: scannerData } = hookContainer('/get-scanner-list');
    ...
    const ColumnHeader = [
    { field: 'scannerName', headerName: 'Scanner Name', width: 450,
      renderCell: (params) => (
    ...
    { field: "action", headerAlign: 'right',
      ...
      renderCell: (data) => {
          const SelectData = () => {
            const obj = {
              id: data.row.id,
              name: data.row.scannerName,
              path: data.row.dedicatedTempPath,
            };
  ...
  const refreshData = () => queryClient.invalidateQueries(['/get-scanner-list']);
  ...
  const deleteData = useMutation({
    mutationFn: () => http.delete(`/delete-scanner?id=${selectedID}`),
    onSuccess: () => {
  ...
  return (
    <>
    <AddScanner RefreshData={refreshData} />
...
export default ScannerListData