...
const ActionModal = ({ RefreshData }) => {
    ...
    return (
        <CustomDialog
            ...
            DialogContents={
                <Box sx={{ mt: 1 }}>
                    <TextField label="Chapa ID" value={client_code} onChange={(e) => { setClientCode(e.target.value) }} fullWidth sx={{ mt: 1 }} size="medium" />
                    <TextField label="Last Name" value={client_name} onChange={(e) => { setClientName(e.target.value) }} fullWidth sx={{ mt: 1 }} size="medium" />
                    <TextField label="First Name" value={client_address} onChange={(e) => { setClientAddress(e.target.value) }} fullWidth sx={{ mt: 1 }} size="medium" />
                    <TextField label="Middle Name" value={client_contactno} onChange={(e) => { setContactNo(e.target.value) }} fullWidth sx={{ mt: 1 }} size="medium" />
                    <TextField label="Ext. Name" value={client_email} onChange={(e) => { setEmail(e.target.value) }} fullWidth sx={{ mt: 1 }} size="medium" />
                    <TextField label="Activity" value={client_tinno} onChange={(e) => { setTINNo(e.target.value) }} fullWidth sx={{ mt: 1 }} size="medium" />
                </Box>
            }
            DialogAction={
